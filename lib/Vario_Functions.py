...
def analysis_button(self, mainUI, canvasVario):
	...
	for i in range(len(bin_no)):
		for j in range(len(bin_space)):
			...
			mainUI.fit_model = Gaussian(dim = 2)
			...
			mainUI.fit_model_x = Gaussian(dim = 2)
			mainUI.fit_model_y = Gaussian(dim = 2)
...
def calculate_best_params(mainUI):
	...
	mean_fit_var 		= np.mean(fit_var[(fit_var <= 10)])
	mean_fit_len 		= np.mean(fit_len[(fit_len <= 50)])
	mean_fit_var_x 		= np.mean(fit_var_x[(fit_var_x <= 10)])
	mean_fit_len_x 		= np.mean(fit_len_x[(fit_len_x <= 50)])
	mean_fit_var_y 		= np.mean(fit_var_y[(fit_var_y <= 10)])
	...
	if mainUI.vtkMode == 1:
		...
		scaled_fit_len_y    = mean_fit_len_y * float(mainUI.DB[mainUI.ID]["Downsample Y"])
